    ...
    <div class="col-md-6 mx-auto">
        <form method="post">
            ...
            <?php if(!empty($row)) :?>
                ...
                <?php endif;?>
                <div class="form-floating">
                    <div class="form-control" id="floatingInput"><?=old_value("category", $row['category'])?></div>
                    <label for="floatingInput">Category</label>
                    <?php if (!empty($errors['category'])):?>
                        <div class="text-danger"><?=$errors['category']?></div>
                    <?php endif;?>
            ...
            <?php if (!empty($rows)):?>
                <?php foreach($rows as $row):?>
                    <tr>
                        ...
                        <td><?=$row['category']?></td>
                        <td><?=$row['slug']?></td>
<!--                        <td>--><?php //=$row['disabled']?><!--</td>-->
                        <td>
                            <a href="<?=ROOT?>/admin/categories/edit/<?=$row['id']?>">
                                <button class="btn btn-warning text-white btn-sm"><i class="bi bi-pencil-square"></i></button></a>
                            <a href="<?=ROOT?>/admin/categories/delete/<?=$row['id']?>">
                                <button class="btn btn-danger btn-sm"><i class="bi bi-trash-fill"></i></button></a>
                        </td>
                    </tr>
                <?php endforeach;?>
            <?php endif;?>
